...
public class MixinTacAimingHandler {
    @ModifyReturnValue(method = "getAlphaProgress", at = @At("RETURN"), require = 0)
    private float lockSpeedAtSsMode_103(float original, GunData gunData) {
    ...
    public static class MixinFovEvent {
        ...
        private static float redirectFov(IGun gun, ItemStack stack, Operation<Float> original) {
            if (CONSTANT_AIMING_ZOOM_SCALE.get() && IShoulderSurfing.getInstance().isShoulderSurfing()) {
                return CONSTANT_AIMING_ZOOM_SCALE_VALUE.get().floatValue();
            }
            return original.call(gun, stack);
        }
    }
}
